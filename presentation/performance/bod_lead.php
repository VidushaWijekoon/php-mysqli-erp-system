...
$date = $date1->format('Y-m-d 00:00:00');
$date2 = $date1->format('Y-m-d 23:59:59');
$query = "SELECT qr_number FROM performance_record_table WHERE (job_description='store to bodywork rack') AND start_time between '$date'AND '$date2' ORDER BY performance_id DESC";
...
foreach ($query_run as $data) {
    ...
    ?>
                    <tr>
                        <td><?php echo $received_qr ?></td>
                        <?php
$query = "SELECT technician_id FROM performance_record_table WHERE qr_number='$received_qr' AND (job_description='send to Bodywork' OR job_description='send to Sanding' OR job_description='send to Taping') AND start_time between '$date'AND '$date2' ORDER BY performance_id DESC";
    $query_run1 = mysqli_query($connection, $query);
    $technician_id = 'not distributed';
    if (empty($query_run1)) {
    } else {
