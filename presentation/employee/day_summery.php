                ...
                <div class="card-body">
                    ...
                    <table id="example2" class="table table-bordered table-striped">
                        <table id="tblexportData" class="table table-striped">
                            ...
                            <tbody>
                                <?php
                                $query = "SELECT job_description, start_time,COUNT(qr_number) AS qr_number 
                                    FROM performance_record_table 
                                    WHERE user_id=$user_id AND start_time BETWEEN $from_date AND $to_date GROUP BY job_description, CAST(start_time AS DATE) ORDER BY start_time";
                                $query_run = mysqli_query($connection, $query);
                                // $row = mysqli_num_rows($query_run);
                                foreach ($query_run as $a) {
                                    $check_date = $a['start_time'];
                                    $date = $a['start_time'];
                                    $date = strtotime($date);
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td>
                                            <?php
                                            $q1 = "SELECT job_description, start_time FROM performance_record_table 
                                                    WHERE user_id=$user_id AND start_time = '$check_date' GROUP BY job_description";
                                            $q2 = mysqli_query($connection, $q1);
                                            while ($xd = mysqli_fetch_assoc($q2)) {
                                                $jobs = $xd['job_description'];
                                                echo $jobs;
                                            }
                                            ?>
